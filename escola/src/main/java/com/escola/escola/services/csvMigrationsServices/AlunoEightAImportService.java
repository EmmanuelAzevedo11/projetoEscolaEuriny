...
@Component
public class AlunoEightAImportService implements CommandLineRunner {

    private final UserRepository userRepository;
    private final PasswordEncoder passwordEncoder;

    public AlunoEightAImportService(UserRepository userRepository, PasswordEncoder passwordEncoder){
    ...
    public void run(String ...args) throws Exception{
        boolean response = userRepository.existsByTeam("8a");
        ClassPathResource resource = new ClassPathResource("csvMigrations/listaAlunosEightA.csv");
        if(response == false) {
                    ...
                    if (colunas.length >= 3) {
                        ...
                        // Importa apenas alunos ativos
                            if (userRepository.findByRa(ra).isEmpty()) {
                                ...
                                aluno.setTeam("8a"); // Seta ANTES do save
