...
@Component
public class AlunoEightBImportService implements CommandLineRunner {

    private final UserRepository userRepository;
    private final PasswordEncoder passwordEncoder;

    public AlunoEightBImportService(UserRepository userRepository, PasswordEncoder passwordEncoder){
    ...
    public void run(String ...args) throws Exception{
        boolean response = userRepository.existsByTeam("8b");
        ClassPathResource resource = new ClassPathResource("csvMigrations/listaAlunosEightB.csv");
        if(response == false) {
                    ...
                    if (colunas.length >= 3) {
                        ...
                        // Importa apenas alunos ativos
                            if (userRepository.findByRa(ra).isEmpty()) {
                                ...
                                aluno.setTeam("8b"); // Seta ANTES do save
