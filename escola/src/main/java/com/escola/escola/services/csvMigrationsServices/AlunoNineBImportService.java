...
@Component
public class AlunoNineBImportService implements CommandLineRunner {

    private final UserRepository userRepository;
    private final PasswordEncoder passwordEncoder;

    public AlunoNineBImportService(UserRepository userRepository, PasswordEncoder passwordEncoder){
    ...
    public void run(String ...args) throws Exception{
        boolean response = userRepository.existsByTeam("9b");
        ClassPathResource resource = new ClassPathResource("csvMigrations/listaALunosNineB.csv");
        if(response == false) {
                    ...
                    if (colunas.length >= 3) {
                        ...
                        // Importa apenas alunos ativos
                            if (userRepository.findByRa(ra).isEmpty()) {
                                ...
                                aluno.setTeam("9b"); // Seta ANTES do save
