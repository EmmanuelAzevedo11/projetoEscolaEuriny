...
@Component
public class AlunoSevenBImportService implements CommandLineRunner {

    private final UserRepository userRepository;
    private final PasswordEncoder passwordEncoder;

    public AlunoSevenBImportService(UserRepository userRepository, PasswordEncoder passwordEncoder){
    ...
    public void run(String ...args) throws Exception{
        boolean response = userRepository.existsByTeam("7b");
        ClassPathResource resource = new ClassPathResource("csvMigrations/listaAlunosSevenB.csv");
        if(response == false) {
                    ...
                    if (colunas.length >= 3) {
                        ...
                        // Importa apenas alunos ativos
                            if (userRepository.findByRa(ra).isEmpty()) {
                                ...
                                aluno.setTeam("7b"); // Seta ANTES do save
