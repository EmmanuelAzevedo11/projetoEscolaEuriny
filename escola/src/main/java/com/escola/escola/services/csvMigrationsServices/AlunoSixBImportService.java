...
@Component
public class AlunoSixBImportService implements CommandLineRunner {

    private final UserRepository userRepository;
    private final PasswordEncoder passwordEncoder;

    public AlunoSixBImportService(UserRepository userRepository, PasswordEncoder passwordEncoder){
    ...
    public void run(String ...args) throws Exception {
        boolean response = userRepository.existsByTeam("6b");
        ClassPathResource resource = new ClassPathResource("csvMigrations/listaAlunosSixB.csv");
        if(response == false){
                    ...
                    if (colunas.length >= 3) {
                        ...
                        // Importa apenas alunos ativos
                            if (userRepository.findByRa(ra).isEmpty()) {
                                ...
                                aluno.setTeam("6b"); // Seta ANTES do save
